...
// Context 는 React에서 컴포넌트 계층 구조(트리)를 통해 데이터를 효율적으로
// 전달하기 위한 매커니즘.
...
export const AuthProvider = ({ children }) => {
  ...
  const handleLogin = async (e) => {
    ...
    // localStorage : 브라우저를 닫아도 데이터가 영구적으로 유지
    // 브라우저 전역에서 사용(모든탭과 창에서 공유됨)
    // 유효기간 만료기능 없음

    // sessionStorage : 브라우저 탭 또는 창을 닫으면 데이터가 즉시 삭제
    ...
    setTimeout(() => {
      // 1시간이 지나면 localStorage 에 있는 데이터 삭제
